...
class TelegramPlatform(crosschat.Platform):
    def __init__(self, crosschat: crosschat.CrossChat, token: str):
        ...
        self.thread = threading.Thread(target=self.app.run_polling, kwargs={"allowed_updates":telegram.Update.ALL_TYPES}, daemon=True)
        self.running = False
    ...
    async def updateData(update: telegram.Update, context: telegram.ext.ContextTypes.DEFAULT_TYPE) -> None:
